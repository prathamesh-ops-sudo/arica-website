...
- **R3FCyberHero** (`@/components/ui/r3f-cyber-hero.tsx`) - React Three Fiber powered 3D hero with Fireship-style scroll-driven camera movements. Features animated cyber grid floor (with offset pattern to avoid center-line artifacts), floating security shields (icosahedrons with fresnel shaders), 600 dynamic data particles, rotating security rings, and mouse parallax effects. Uses @react-three/fiber, @react-three/drei for declarative 3D.
