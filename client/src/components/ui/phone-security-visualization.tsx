...
export function PhoneSecurityVisualization({ isScanning, onThreatBlocked }: PhoneSecurityVisualizationProps) {
  ...
  return (
    <div className="relative w-full h-[500px] rounded-2xl overflow-hidden bg-gradient-to-b from-slate-900/80 to-slate-950/80 border border-[#42BA90]/20">
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-4 bg-black/60 backdrop-blur-xl rounded-full px-4 py-2 border border-[#42BA90]/30">
        <button 
          onClick={() => cycleDevice(-1)}
          className="p-2 hover:bg-[#42BA90]/20 rounded-full transition-colors"
          data-testid="btn-prev-device"
        >
          <svg className="w-5 h-5 text-[#3D70B7]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
          </svg>
        </button>
        
        <div className="text-center min-w-[140px]">
          <div className="text-[#3D70B7] font-medium text-sm">{selectedDevice.name}</div>
      ...
      <div className="absolute top-4 left-4 flex flex-col gap-2">
        <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
          isScanning ? 'bg-[#42BA90]/20 text-[#3D70B7] border border-[#42BA90]/50' : 
          attackMode ? 'bg-red-500/20 text-red-400 border border-red-500/50' :
          'bg-gray-800/60 text-gray-400 border border-gray-600/30'
        }`}>
          <span className={`w-2 h-2 rounded-full ${isScanning ? 'bg-[#3D70B7] animate-pulse' : attackMode ? 'bg-red-400 animate-pulse' : 'bg-gray-500'}`} />
      ...
      {/* Hotspot info toast */}
      {hotspotInfo && (
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-black/80 backdrop-blur-xl px-4 py-2 rounded-lg border border-[#42BA90]/50 text-[#3D70B7] text-sm">
          {hotspotInfo}
        </div>
      )}
    </div>
