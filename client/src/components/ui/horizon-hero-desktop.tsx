...
export function HorizonHeroSection() {
  ...
  useEffect(() => {
    if (!canvasRef.current || isMobileOrTablet || window.innerWidth < 1024) return;
    ...
    const animate = (timestamp: number) => {
      ...
      refs.stars.forEach((starField) => {
        if ((starField.material as any).uniforms) {
          (starField.material as any).uniforms.time.value = time;
        }
      });

      if (refs.nebula && (refs.nebula.material as any).uniforms) {
        (refs.nebula.material as any).uniforms.time.value = time * 0.3;
      }

      refs.distantBodies.forEach((body) => {
        if ((body.material as any).uniforms) {
          (body.material as any).uniforms.time.value = time;
        }
  ...
  useEffect(() => {
    if (!isReady) return;
    let disposed = false;
    let tl: { kill: () => void; from: (...args: any[]) => unknown } | undefined;
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      rafIdRef.current = requestAnimationFrame(() => {
        ...
        if (refs.nebula && (refs.nebula.material as any).uniforms) {
          (refs.nebula.material as any).uniforms.scrollProgress.value = progress;
        }
      });
    };
      ...
      <div className="horizon-hero-content z-[10]" style={{ opacity: isPastHero ? 0 : 1, transition: 'opacity 0.3s ease' }}>
        {currentSection === 0 ? (
          ...
          <h2
            className="horizon-hero-title text-halo-white"
            style={{
              fontSize: isMobileOrTablet ? 'clamp(1.2rem, 5vw, 2rem)' : 'clamp(1.8rem, 4vw, 4rem)',
              whiteSpace: isMobileOrTablet ? 'normal' : 'nowrap',
            }}
          >
            Security That Goes Beyond Prevention
          </h2>
